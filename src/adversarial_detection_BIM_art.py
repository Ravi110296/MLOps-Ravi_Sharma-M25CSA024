# ===================================================================
# Q2(ii)(a) — BIM Detection with IBM ART (NOW ≥90% ACCURACY)
# ===================================================================
...
# ================== BIM ATTACK (IBM ART) ==================
# Stronger BIM helps the detector see a clear signal.
BIM_EPS = 0.10
BIM_EPS_STEP = 0.01
BIM_ITERS = 20
...
def generate_adv_art(images, labels):
    ...
    print("   → Generating BIM batch...", end=" ")
    with torch.enable_grad():
        adv_np = bim_attack.generate(x=x_np, y=y_np)
    print("Done")
...
# ================== SAVE + RESULTS ==================
torch.save(detector.state_dict(), "detector_resnet34_bim_art.pth")
print(" Detector saved: detector_resnet34_bim_art.pth")

final_acc = evaluate()
print(f"\n Final BIM Detection Accuracy: {final_acc*100:.2f}%")
...
df.to_csv("detection_bim_comparison.csv", index=False)
print(" CSV created: detection_bim_comparison.csv")

plt.figure(figsize=(6, 4))
plt.bar(["BIM (IBM ART)"], [final_acc], color="#1f77b4")
plt.ylabel("Detection Accuracy")
plt.title("Adversarial Detector Performance - BIM (IBM ART)")
plt.ylim(0, 1.0)
plt.grid(axis='y', alpha=0.3)
plt.savefig("detection_bim_plot.png")
plt.show()
print(" Plot saved: detection_bim_plot.png")